...
+ [验证] DOTAI Agent 转发器 `v0.1.7` 已在 ChatGPT 页面验证可用，作为当前可回滚稳定版；参考图上传采用文件输入、粘贴、拖拽三路兜底，并在发送提示词前确认目标站点出现新增参考图。
+ [新增] 图片生成器支持选择 ChatGPT Image / Gemini Image 浏览器插件模型，将提示词与参考图转发到目标网站并把生成图片回填画布。
+ [新增] 提供 Agent 对话只读导出桥接，支持浏览器扩展一键转发到 ChatGPT 与 Gemini。
+ [新增] 画布空白区域右键菜单支持添加图片生成器与视频生成器。
